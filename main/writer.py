...
def extend_day_columns(sheet, num_copies, is_last_quarter=False, has_exam=False):
    ...
    # widths = []
    # for i in range(quarter_to_dates_offset):
    #     letter = get_column_letter(i + 4)
    ...
    while col_idx < final_idx:
        ...
        # sheet.column_dimensions[current_col_letter].width = widths[i]
        col_idx += 1
        i += 1
...
def print_widths(sheet, message):
    ...
    for i in range(1, sheet.max_column):
        ...
        width = sheet.column_dimensions[letter].width
        print(f"column at {letter} has width {width}")
    print("\n")
